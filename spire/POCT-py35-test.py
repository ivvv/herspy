...
def calcCorrection(sourceModel,jarFile, plotIt=0):
    ...
    for arr in ["SSW","SLW"]:
        ...
        sourceImage3d = np.repeat(sourceModel[np.newaxis,:,:],nfq,axis=0)
        corr = np.sum(beamImage*sourceImage3d,axis=(1,2))
        #corr = np.zeros(nfq)
        #for i in np.arange(nfq):
...
# 
jar_file = os.path.join(os.path.expanduser('~'),'Dropbox/Work/SPIRE/spire_cal_14_3.jar')
#
